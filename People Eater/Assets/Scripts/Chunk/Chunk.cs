...
public class Chunk : MonoBehaviour
{
    ...
    void Create()
    {
        ...
        // Раздача информации о том, какая будет скорость
        // (эта функция мало настроенна, но можно к примеру сделать одни чанки медленными, другие быстрые)
        Speed = (100 - Chance) * Random.Range(2.9f, 3.1f);
        Platform.GetInformation(Folder, Snake, Speed);
        ...
        if (Count > 0)
        {
            ...
            for (int i = -72; i < 72; i += 2)
            {
                ...
                if (Random.Range(0, 100) > CurChance - StaticOptions.Difficulty / 25 && SkipSpike < i)
                {
                    CreateSpike(i);
                }
    ...
    void CreateSpike(int i)
    {
        ...
        for (int a = -6; a <= 6; a++)
        {
            ...
            if (Random.Range(0, 100) > CurChance && ObjectCount < 2)
            {
                ...
                Dangers[Dangers.Count - 1].transform.localPosition = new Vector3(Count * 150 + i, 1 + StaticOptions.Difficulty / 100, a);
